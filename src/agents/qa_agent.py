...
def validate(state: PipelineState, bronze: pd.DataFrame, silver: pd.DataFrame,
    ...
    if bronze is not None:
        _check("bronze_has_rows", len(bronze) > 0,
               f"Bronze has {len(bronze)} rows.")
